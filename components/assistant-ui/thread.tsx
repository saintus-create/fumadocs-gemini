    ...
    <div className="aui-thread-welcome-root mx-auto my-auto flex w-full max-w-(--thread-max-width) grow flex-col">
      <div className="aui-thread-welcome-center flex w-full grow flex-col items-center justify-center">
        <div className="aui-thread-welcome-message flex size-full flex-col justify-center px-4">
          ...
          <p className="aui-thread-welcome-message-inner fade-in slide-in-from-bottom-1 animate-in fill-mode-both text-base text-muted-foreground delay-75 duration-200">
            Start with a question and follow the answer back into the source pages when you need detail.
          </p>
        </div>
      </div>
      <ThreadSuggestions />
  ...
  return (
    <div className="aui-thread-welcome-suggestions grid w-full @md:grid-cols-2 gap-2 pb-4">
      <ThreadSuggestionItem
        prompt="What should I read first in these docs?"
        description="Start here"
      />
      <ThreadSuggestionItem
        prompt="Summarize the most important concepts on this site."
        description="Big picture"
      />
      <ThreadSuggestionItem
        prompt="Show me the docs pages most relevant to setup."
        description="Setup pages"
      />
      <ThreadSuggestionItem
        prompt="What workflows are covered in these docs?"
...
const ThreadSuggestionItem: FC<{ prompt: string; description: string }> = ({ prompt, description }) => {
  return (
    <div className="aui-thread-welcome-suggestion-display fade-in slide-in-from-bottom-2 @md:nth-[n+3]:block nth-[n+3]:hidden animate-in fill-mode-both duration-200">
      <ThreadPrimitive.Suggestion
        ...
        render={
          <Button
            variant="ghost"
            className="aui-thread-welcome-suggestion h-auto w-full @md:flex-col flex-wrap items-start justify-start gap-1 rounded-3xl border bg-background px-4 py-3 text-left transition-colors hover:bg-muted"
          />
        }
      >
        <span className="aui-thread-welcome-suggestion-text-1 text-sm font-medium">{description}</span>
        <span className="aui-thread-welcome-suggestion-text-2 text-sm text-muted-foreground">{prompt}</span>
      </ThreadPrimitive.Suggestion>
    </div>
  );
};
...
const AssistantMessage: FC = () => {
  return (
    <MessagePrimitive.Root
      ...
      data-role="assistant"
    >
      <div className="aui-assistant-message-content wrap-break-word px-2 text-base text-foreground">
  ...
  return (
    ...
    >
      ...
      <ActionBarMorePrimitive.Root>
        <ActionBarMorePrimitive.Trigger className="inline-flex size-8 items-center justify-center rounded-md transition-colors hover:bg-accent data-[state=open]:bg-accent">
  ...
  return (
    ...
    >
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className="aui-user-message-content wrap-break-word peer rounded-2xl bg-muted px-4 py-2.5 text-base text-foreground empty:hidden">
          <MessagePrimitive.Parts />
        </div>
